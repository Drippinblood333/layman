---
name: layman-router
description: Install, configure, start, verify, recover, and explain Layman's local API routing service for Codex. Use when the user asks to enable model auto-routing, open the management dashboard, preview or restore Codex provider configuration, check router health, inspect cost and fallback reports, or troubleshoot the local OpenAI-compatible Responses proxy.
---

# Layman Router Operations

Layman Router is the API-user layer of Layman 1.0. It runs as a loopback-only local service, keeps explicit models unchanged, and routes `model="auto"` through configurable fast, balanced, or deep tiers. ChatGPT Plus users use `$layman-auto` instead; their subscription task launcher does not enable this API-backed service.
...
2. Check whether the service is installed and run `layman doctor` before changing configuration.
3. When enabling Codex integration, always run `layman codex enable --dry-run` first and show the diff. Run `--apply` only after the user explicitly asks to apply it.
...
6. Use `layman report` after setup. Explain that estimated savings are counterfactual; measured savings require a real API benchmark.
7. Use `layman codex disable --apply` to restore the prior provider settings. Do not hand-edit or delete the user's entire Codex config.
