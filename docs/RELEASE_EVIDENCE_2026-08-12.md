...
- The hashed runtime lock installed cleanly under Python 3.14 and the independent bundled Python 3.12.13 runtime. Python 3.12 then passed `pip check`, `doctor`, and all 90 tests. Hash enforcement rejected inconsistent proxy/cache responses; a cache-free download matched the official PyPI hashes and passed. Python 3.11 remains a hosted-CI gate.
