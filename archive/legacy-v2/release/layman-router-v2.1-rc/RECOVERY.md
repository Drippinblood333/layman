...
1. Stop Layman Router and run `layman-router codex disable --dry-run`.
2. Review the diff, then run `layman-router codex disable --apply`.
3. If the restore-state file is missing or a conflict is reported, list backups with `layman-router codex backups`.
4. Preview a selected backup with `layman-router codex restore <backup-path> --dry-run`.
...
Run `layman-router doctor`. Either stop the stale process or set `LAYMAN_ROUTER_PORT` and update the Codex provider URL consistently. Do not bind the service to a LAN address outside an isolated container.
...
Stop the router and copy `~/.layman-router/usage.sqlite3` before repair. The service can start against a new path by setting `LAYMAN_ROUTER_DATABASE_PATH`; do not delete the original until its data is no longer needed.
